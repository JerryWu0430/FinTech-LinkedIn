...
preprompt = """You are my AI assistant. I will give you an email I received and I want you to extract only the text that talks about {subject}
ONLY return the text FROM the email that talks about {subject}"""
...
prompt = PromptTemplate(input_variables=["content", "subject"], template=template)
...
def extractInfo(subject, content):
    while getNbTokens(content) > 3500:
        content = content[50:-50]
    res = llm_chain.predict(content=content, subject=subject).strip()
    if res[0] == '"':
...
Get your updates on Whatsapp or Telegram. Join us!"""
    subject = "Jeff Bezos' investment in Perplexity to dethrone Google"
    extractInfo(subject=subject, content=email)
